...
my $root_dir = $ENV{"BASICEVENTSYNTAXCHECKER_PRJ_ROOT"};
$root_dir =~ s/\\/\//g;
...
# Copy executables in $BASICEVENTHANDLING_ROOT/bin and $BASICEVENTHANDLING_ROOT/bin/$(ConfigurationName)
my $ft = File::Type->new();

my $basic_event_handling_root = $ENV{BASICEVENTHANDLING_PRJ_ROOT};
my $debug_dir = "$root_dir/bin/Debug";
my $release_dir = "$root_dir/bin/Release";
...
if (DPOUtils::TreeScan($root_dir, \@files))
{
    foreach my $f (@files)
    {
        if (DPOUtils::is_executable($f, $ft))
        {
...
if (!DPOPostbuild::dpo_postbuild("BasicEventSyntaxChecker"))
{
    exit -1;
}
